...
import { TENANT_COOKIE, THEME_COOKIE, TOKEN_COOKIE } from '$lib/server/session';

export const handle: Handle = async ({ event, resolve }) => {
	event.locals.token = event.cookies.get(TOKEN_COOKIE);
	event.locals.tenantSlug = event.cookies.get(TENANT_COOKIE);

	const chosen = event.cookies.get(THEME_COOKIE);
	event.locals.theme = chosen === 'light' || chosen === 'dark' ? chosen : 'system';
