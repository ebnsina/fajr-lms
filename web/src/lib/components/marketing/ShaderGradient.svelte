<script lang="ts">
	...
	const fragment = `
		...
		void main() {
			...
			vec3 color = mix(base, deep, smoothstep(0.0, 1.4, uv.y));

			color = mix(color, glow, ribbon(uv, 0.62, 0.09, 0.30) * 0.85);
			color = mix(color, glow, ribbon(uv, 0.38, 0.13, 0.22) * 0.55);
			color = mix(color, deep, ribbon(uv, 0.82, 0.07, 0.26) * 0.60);

			// A wide, low pool of light behind the middle of the text.
			float pool = smoothstep(0.9, 0.0, length((p - vec2(aspect * 0.5, 0.5)) * vec2(0.55, 1.2)));
			color = mix(color, glow, pool * 0.35);
